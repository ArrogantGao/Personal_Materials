...
#let manual-square(vertice, radius:0.2) = {
  import draw: *
  let (i, j) = vertice
  let ld = (i - radius, j - radius)
  let rd = (i + radius, j - radius)
  let lu = (i - radius, j + radius)
  let ru = (i + radius, j + radius)
  line(ld, lu, ru, rd, close:true, fill:white, stroke:black)
}
...
#let show-graph-content(vertices, edges, names, radius:0.2, fontsize:10pt) = {
  import draw: *
  for (k, (i, j)) in vertices.enumerate() {
    circle((i, j), radius:radius, name: str(k), fill:black)
  }
  // for (k, l) in edges {
  //   line(str(k), str(l))
  // }
  for (k, i) in names {
    content(str(k), text(white, fontsize)[#i])
  }
}

#let show-graph-black(vertices, edges, radius:0.2) = {
  import draw: *
  for (k, (i, j)) in vertices.enumerate() {
    circle((i, j), radius:radius, name: str(k), fill:black)
  }
